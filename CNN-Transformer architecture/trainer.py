...
def evaluate(subset, encoder, decoder, config, device, save_path="visualizations"):
    ...
    for batch_idx, (x_img, y_caption) in enumerate(subset.inference_batch(batch_size)):
        ...
        # Visualization for the first 5 images in the batch
        if batch_idx == 0:
            # transform = transforms.ToPILImage()
            # for i in range(min(5, len(x_img))):
            #     img = transform(x_img[i].cpu())  # Convert tensor to PIL image
            #     predicted_caption = predictions[i]
            ...
            for i in range(min(5, len(x_img))):
                img = reverse_normalize(x_img[i].cpu())
                img = torch.clamp(img, 0, 1)  # Ensure pixel values are in the valid range [0, 1]
                ...
                plt.axis("off")
                plt.savefig(os.path.join(save_path, f"image_{i+1}.png"))
                plt.close()
                # # Save the image with caption
                # plt.figure(figsize=(8, 8))
                # plt.imshow(img)
                # plt.title(f"Predicted: {predicted_caption}", fontsize=12)
                # plt.axis('off')
                # plt.tight_layout()
                # plt.savefig(os.path.join(save_path, f"image_{i+1}.png"))
                # plt.close()
